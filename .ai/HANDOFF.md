...
- **Next Up:** **Milestone 21 — Tax Master & HSN / CGST-SGST-IGST Split**, whose dependency is now satisfied (M19 supplies `Branch.stateCode`, and `isIntraState()` already exists in `src/lib/branch.ts`). Read `TODO.md`'s **Sequencing Note** first: M48 (Rate Master) is still a live D-4 compliance defect and warrants pulling forward. The money-as-float fix should also land before the accounting phases. Note M21 will have to confront the diamond-HSN ambiguity in §1 above, which still needs CA sign-off.
