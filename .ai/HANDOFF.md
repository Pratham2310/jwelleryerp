...
- **✅ Done (2026-07-29): Milestone 25 — Non-hallmarked sale prevention guard.** Configurable block/warn/off gate with the §11.3 exemptions (metal, category, weight, shop turnover), a live banner while billing, and detection kept separate from enforcement. Test suite now **497 passing across 21 suites**. **Phase 9's BIS hallmarking pair is complete.**
- **Next Up:** **Milestone 26 — Gold Savings Scheme master & enrolment (PRD §12).** Note `PROJECT_OVERVIEW.md` rule 6 and the Handbook Phase 10: the BUIDS Act **hard-blocks a cash refund** of scheme balances, which the current Scheme Redemption path already respects as a pure spend-down — keep it that way. The money-as-float fix should still land before the accounting phases (M38+).
