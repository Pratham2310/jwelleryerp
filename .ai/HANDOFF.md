...
- **Next Up:** **Milestone 24 — BIS Hallmarking & HUID assignment workflow.** `Tag.huid` exists and prints correctly, but only manual entry populates it; there is no AHC dispatch/receipt flow. Read `TODO.md`'s **Sequencing Note** first: M48 (Rate Master) is still a live D-4 defect for *metal* rates and warrants pulling forward — the Tax Master built in M21 is now a working reference for exactly that append-only, effective-dated pattern. The money-as-float fix should also land before the accounting phases.
