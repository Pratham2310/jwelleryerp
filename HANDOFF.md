...
- **Next Up:** **Milestone 24 — BIS Hallmarking & HUID assignment workflow.** `Tag.huid` exists and prints correctly, but only manual entry populates it; there is no AHC dispatch/receipt flow, and PRD Phase 9 also wants a non-hallmarked sale block. The money-as-float fix should still land before the accounting phases (M38+).
