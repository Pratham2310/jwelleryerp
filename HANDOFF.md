...
- **Next Up:** **Milestone 25 — Non-hallmarked sale prevention guard (PRD §11.3).** Its dependency is now satisfied: real HUIDs exist, but a piece without one can still be billed. Note §11.3 wants this **configurable as hard-block vs warning**, because genuine exemptions exist (pieces under 2g, certain categories, shops below the turnover threshold, antiques and exports) — so do not make it an unconditional block. The money-as-float fix should still land before the accounting phases (M38+).
