...
- **Next Up:** **Milestone 29 — Tally Prime export.** A downloaded XML file only, per the simulation ground rule in `.ai/IMPLEMENTATION_WORKFLOW.md` — no real Tally integration. It now has complete books to export; before procurement existed it would have exported only the sales half. After that, Phase 14 (M45–47) makes the P&L and Balance Sheet meaningful, since purchases finally exist to cost against revenue.
